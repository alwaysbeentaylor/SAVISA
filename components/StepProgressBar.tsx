        ...
        {STEPS.map((step, index) => (
          <React.Fragment key={step}>
            <div className="flex flex-col items-center relative flex-1">
              <div className={`w-10 h-10 rounded-full flex items-center justify-center border-2 transition-all duration-300 ${index < currentStep ? 'bg-emerald-600 border-emerald-600 text-white' :
              ...
              <span className={`mt-2 text-xs font-medium uppercase tracking-wider hidden md:block ${index <= currentStep ? 'text-emerald-700' : 'text-slate-400'
                }`}>
                {step}
              </span>
            </div>
